...
**Build scripts:** `scraper-sofascore.js` (primary), `scraper-data.js` (fallback), `scraper-kits.js`, `scraper-kits-wc.js`, `scraper-injuries.js` — Node 18+, no external deps (scraper-data.js requires API key in `.env`)  
...
**Primary script:** `scraper-sofascore.js` — uses Sofascore's public API. No key required. Covers all three EFL divisions + World Cup.

```
node scraper-sofascore.js --comp EL1 --date 2026-08-10   # list fixtures on a date
node scraper-sofascore.js --match 14060002                # fetch full data for one match
```
...
- Sofascore also provides team colours (hex) and crest image URLs — used in banner

**Sofascore competition IDs:**
- Championship: tournament `18`, season 25/26 = `77347`
- League One: tournament `24`, season 25/26 = `77352`
- League Two: tournament `25`, season 25/26 = `77351`
